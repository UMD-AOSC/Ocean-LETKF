...
PYTHONPATH=../../pycommon ./get_l3_smap_jpl.py --start_date 20220101 --end_date 20220102 --outdir ./ --topdir_name test_l2_sss --skip_remote_check --verbose --log
"""
...
parser = argparse.ArgumentParser(description=("Script to download JPL L3 SMAP 8-day-running mean SSS"),
...
parser.add_argument("--topdir_name", default="l3_smap_jpl", required=False, help=("top directory name for saving all data"))
...
if args.log:
    logging = common.setupEasyLogging("get_l3_smap_jpl")
else:
    logging = common.setupEasyLogging("get_l3_smap_jpl",fileLog=False)
...
while cdate <= args.end_date:
    ...
    if not args.skip_remote_check:
        ...
        cmd_inq = 'wget -r -l1 -np -nH --spider --cut-dirs 20 "https://opendap.jpl.nasa.gov/opendap/allData/smap/L3/JPL/V5.0/8day_running/{:04d}/{:03d}/" -A "SMAP*.nc" -P ./ -e robots=off'.format(cdate.year, julian_day)
        if args.verbose:
    ...
    cmd_get = 'wget -r -l1 -np -nH --cut-dirs 20 "https://opendap.jpl.nasa.gov/opendap/allData/smap/L3/JPL/V5.0/8day_running/{:04d}/{:03d}/" -A "SMAP*.nc" -P ./ -e robots=off'.format(cdate.year, julian_day)
    if args.verbose:
