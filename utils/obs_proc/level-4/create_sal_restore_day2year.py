...
def parseCommandLine():
    parser = argparse.ArgumentParser(description=("merge daily salinity MOM files to yearly file"))
...
def main(args):
    ...
    while cdate <= args.end_date:
        ...
        if cdate == args.start_date:
            ...
            ndays = (args.end_date - args.start_date).days + 1
            sal_year = np.zeros((ndays, nlat, nlon))
            day_year = np.zeros(ndays)
            base_date = dt.datetime(1900,1,1,0,0,0)
       
        idx_day = (cdate - args.start_date).days
        sal_year[idx_day,:,:] = wk2d
        day_year[idx_day] = (cdate - base_date).days

        cdate += dt.timedelta(days=1)

    f = Dataset(args.template_file_path,"r+")
    f.variables["SALT"][:] = sal_year.astype(np.float32)
    f.variables["time"][:] = day_year.astype(np.float32)
    f.close()
