...
# cp -f $SBCDIR/input.nml ${workdir2}/INPUT/
  ...
  if [ "$datype" == "GODAS" ]; then # GODAS GODAS GODAS !!!
    ...
    #STEVE: for using IAU, we want the analysis increment as a "correction" file:
    if [ $USE_IAU -eq "1" ]; then
      if [ -f $workdir_analysis/RESTART/ocean_cor.res.nc ]; then
        ln -f $workdir_analysis/RESTART/ocean_cor.res.nc ${workdir2}/INPUT/ocean_cor.res.nc
      else
       echo "ERROR: ANALYSIS Increment Correction FILE DOES NOT EXIST: $workdir_analysis/RESTART/ocean_cor.res.nc"
        exit 1
      fi
      # Use the model forecast as initial conditions instead of the godas analysis.
      # The analysis will be applied in increments within the mom4p1 forecast run.
      echo "Using Model forecast from previous step as initial conditions for ocean_temp_salt ..."
