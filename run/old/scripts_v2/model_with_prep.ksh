...
workdir_prep=${EXP_DATA}/${YYYYMMDDHH}/model_prep/${MEMBERID}
workdir_model=${EXP_DATA}/${YYYYMMDDHH}/model/${MEMBERID}
mkdir -p ${workdir_prep}
...
mkdir -p ${workdir_model}/INPUT
mkdir -p ${workdir_model}/RESTART
...
if [ "$isfirst" -eq "1" ]; then
  ...
  ln -f $INPUT_INIT/* ${workdir_model}/INPUT/
else
  ...
  ln -f $INPUT_INIT/* ${workdir_model}/INPUT/
# ln -fs $EXP_DATA/INPUT/chl.nc ${workdir_model}/INPUT  
# ln -fs $EXP_DATA/INPUT/grid_spec.nc ${workdir_model}/INPUT  
# ln -fs $EXP_DATA/INPUT/namelist ${workdir_model}/INPUT  
# ln -fs $EXP_DATA/INPUT/ncar_precip_clim.nc ${workdir_model}/INPUT  
# ln -fs $EXP_DATA/INPUT/RUNOFF.nc ${workdir_model}/INPUT  
# ln -fs $EXP_DATA/INPUT/salt12.nc ${workdir_model}/INPUT  
# ln -fs $EXP_DATA/INPUT/SNOW.nc ${workdir_model}/INPUT  
# ln -fs $EXP_DATA/INPUT/sst_ice_clim.nc ${workdir_model}/INPUT  
# ln -fs $EXP_DATA/INPUT/input.nml ${workdir_model}/INPUT
  rm -f ${workdir_model}/INPUT/input.nml
  cp -f $SBCDIR/input.nml ${workdir_model}/INPUT/
  ...
  do
    echo "linking $file to ${workdir_model}/INPUT ..."
    ln -f $file ${workdir_model}/INPUT
  done
  ...
  rm -f ${workdir_model}/INPUT/ocean_temp_salt.res.nc
  rm -f ${workdir_model}/INPUT/ocean_velocity.res.nc
  rm -f ${workdir_model}/INPUT/ocean_sbc.res.nc
  ...
  if [ "$datype" == "GODAS" ]; then # GODAS GODAS GODAS !!!
    ...
    if [ -f $workdir_analysis/RESTART/ocean_temp_salt.res.nc ]; then
      ln -f $workdir_analysis/RESTART/ocean_temp_salt.res.nc ${workdir_model}/INPUT/ocean_temp_salt.res.nc
    else
    ...
    if [ -f $srcfile ]; then
      ln -f $srcfile ${workdir_model}/INPUT/ocean_velocity.res.nc
    else
    ...
    if [ -f $srcfile ]; then
      ln -f $srcfile ${workdir_model}/INPUT/ocean_sbc.res.nc
    else
      echo "ERROR: ANALYSIS FILE DOES NOT EXIST: $srcfile"
      exit 1
    fi
    ...
    if [ $USE_ALTIMETRY -eq "1" ]; then
      ...
      if [ -f $srcfile ]; then
        ln -f $srcfile ${workdir_model}/INPUT/ocean_barotropic.res.nc
      else
        echo "ERROR: ANALYSIS FILE DOES NOT EXIST: $srcfile"
        exit 1
      fi
    ...
    if [ -f $workdir_analysis/anal${MEM3}.ocean_temp_salt.res.nc ]; then
      ln -f $workdir_analysis/anal${MEM3}.ocean_temp_salt.res.nc ${workdir_model}/INPUT/ocean_temp_salt.res.nc
    else
    ...
    if [ -f $workdir_analysis/anal${MEM3}.ocean_velocity.res.nc ]; then
      ln -f $workdir_analysis/anal${MEM3}.ocean_velocity.res.nc ${workdir_model}/INPUT/ocean_velocity.res.nc
    else
    ...
    if [ -f $workdir_analysis/anal${MEM3}.ocean_sbc.res.nc ]; then
      ln -f $workdir_analysis/anal${MEM3}.ocean_sbc.res.nc ${workdir_model}/INPUT/ocean_sbc.res.nc
    else
    ...
    if [ $USE_ALTIMETRY -eq "1" ]; then
      if [ -f $workdir_analysis/anal${MEM3}.ocean_barotropic.res.nc ]; then
        ln -f $workdir_analysis/anal${MEM3}.ocean_barotropic.res.nc ${workdir_model}/INPUT/ocean_barotropic.res.nc
      else
...
if [ ! -f ${workdir_model}/INPUT/$rtype.res ]; then
  echo "LINK ERROR: ${workdir_model}/INPUT/$rtype.res does not exist. Unfortunately."
  exit 1
fi
...
echo "$IY  $IM  $ID  $IH  $IN  $IS" >> ${workdir_prep}/time_stamp.out
echo "In: $PWD"
echo "Running:"
cp /sw/xe6/wgrib/1.8.1.0b/sles11.1_gnu4.3.4/bin/wgrib ${workdir_prep}
if [ $USE_EFLX -eq 1 ]; then
  echo "${SBCDIR}/$mDS ${workdir_model} $FLXDIR $FLXDIR2 $SSTDIR $days"
  ${SBCDIR}/$mDS ${workdir_model} $FLXDIR $FLXDIR2/$MEM2 $SSTDIR $days
  #STEVE: need to format each ensemble member exactly as R2 (to make my life easier)
elif [ $USE_MFLX -eq 1 ]; then
  echo "${SBCDIR}/$mDS ${workdir_model} $FLXDIR $days"
  ${SBCDIR}/$mDS ${workdir_model} $FLXDIR $SSTDIR $days
fi

# link to each model working directory
for file in `ls -d ${workdir_prep}/*_sfc_restore.nc`; do
  ln -f $file ${workdir_model}/INPUT
done
...
for file in `ls -d ${workdir_prep}/RA2_daily_*.nc`; do
  ln -f $file ${workdir_model}/INPUT
done
...
if [ "$DO_SFCFLUXES" -eq 1 ]; then
  ...
  for file in `ls -d ${workdir_prev}/RA2_daily_*.nc`; do
    ...
    ln -f $file ${workdir_prep}/$newRA2file   #STEVE: this is the original SFC RA2 that should be used for the increment for bias correction
  done
  ...
  if [ -f ${workdir_analysis}/SFA_${MEM3}_daily_pres.nc ]; then
    ln -f ${workdir_analysis}/SFA_${MEM3}_*.nc ${workdir_prep}       #STEVE: this is the SFC 'analysis' generated by LETKF
  ...
  if [ -f SFR_${MEM3}_daily_pres.nc ]; then
    ...
    for file in `ls -d SFR_${MEM3}_*.nc`; do
      ...
      ln -fs $workdir_prep/$file3 ${workdir_model}/INPUT/$file2
    done
  else
  ...
  for file in `ls -d ${workdir_prep}/RA2_daily_*.nc`; do
    ln -fs $file ${workdir_model}/INPUT
  done
fi

# STEVE: prevent error with too many hard links to grid_spec.nc:
rm -f ${workdir_prep}/grid_spec.nc
...
echo $0
if [ ! -f ${workdir_model}/INPUT/ocean_temp_salt.res.nc ]; then
  echo "Input file does not exist... but it should: ${workdir_model}/INPUT/ocean_temp_salt.res.nc"
  exit 1
fi
