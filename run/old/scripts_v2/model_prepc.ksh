...
echo "Model preparation step (compact)"
...
MEMBERS2=`printf %.2d ${MEMBERS}`
if [ "$MEMBERS2" -eq "01" ]; then
  USE_MFLX=1
  USE_EFLX=0
else
  USE_MFLX=0
...
#===============================================================================
# Loop through all members and preprocess model prep data
#===============================================================================
MEM=0
...
do
  ...
  MEM2=`printf %.2d ${MEM}`
  MEM3=`printf %.3d ${MEM}`
  echo "This is the model preparation step for member ${MEM} for cycle ${YYYYMMDDHH}" > model_prep.out
  workdir_prep=${EXP_DATA}/${YYYYMMDDHH}/model_prep/${MEM}
  workdir_model=${EXP_DATA}/${YYYYMMDDHH}/model/${MEM}
...
#   ln -fs $EXP_DATA/INPUT/input.nml ${workdir_model}/INPUT
    ...
    workdir0=${EXP_DATA}/$PY$PM$PD$PH/model/${MEM}
...
if [ "$DO_SFCFLUXES" -eq 1 ]; then
  ...
  if [ -f SFR_${MEM3}_daily_pres.nc ]; then
    ...
    for file in `ls -d SFR_${MEM3}_*.nc`; do
      ...
      ln -fs $workdir/$file3 ${workdir_model}/INPUT/$file2
    done
  else
