...
SUBROUTINE phys2ijk(elem,rlon,rlat,rlev,ri,rj,rk)     !(OCEAN)
  ...
  REAL(r_size) :: aj,ak,ai, rrlon, glon,glat
...
!STEVE: probably want to replace this with a kd-tree lookup for general search (KDTREE)
  ...
  if (initialized == 0) then
    ...
    initialized = 1
    call kd_init( kdtree_root, RESHAPE(lon2d(:,:),(/1/)), RESHAPE(lat2d(:,:),(/1/)) )
    WRITE(6,*) "Done constructing KD search tree."
    WRITE(6,*) "nlon*nlat = ", nlon*nlat
!   ALLOCATE(dist(nlon*nlat))
!   ALLOCATE(idx(nlon*nlat))
  ...
  if (rlon /= prev_lon .or. rlat /= prev_lat) then
    ...
    if (dodebug) then
      ...
      WRITE(6,*) "Observations found by kd_search_nnearest = "
      WRITE(6,*) "idx,ni,nj,lon,lat,dist"
      do n=1,k_found
        ni = MODULO(idx(n)-1,nlon)+1
        nj = idx(n)/nlat ! Integer division
  ...
  nj = idx(1)/nlat ! Integer division
  glon = lon2d(ni,nj)
  glat = lat2d(ni,nj) 
  ...
  elseif (glon < rlon) then 
    ai = (rlon - lon2d(ii,jj)) / (lon2d(ii+1,jj) - lon2d(ii,jj))
  elseif (rlon < glon) then
    ai = (rlon - lon2d(ii,jj)) / (lon2d(ii,jj) - lon2d(ii-1,jj))
  endif
  ri = REAL(ii,r_size) + ai 
  ...
  elseif (glat < rlat) then
    aj = (rlat - lat2d(ii,jj)) / (lat2d(ii,jj+1) - lat2d(ii,jj))
  elseif (rlat < glat) then
    aj = (rlat - lat2d(ii,jj)) / (lat2d(ii,jj) - lat2d(ii,jj-1))
    !STEVE: should be negative
  endif
  rj = REAL(jj,r_size) + aj
