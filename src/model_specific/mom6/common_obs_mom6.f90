...
!STEVE:MOM6: these are still set for mom4p1:
...
! ks = ke-1
  ...
  CASE(id_eta_obs) ! SSH                          !(OCEAN)
    ...
    if (Aflag .eq. 1) then ! GODAS-type altimetry assimilation
      ...
      ! The model climatology is removed from the eta field when reading in common_mom6.f90
...
!
  ...
  ELSE
    ...
    if (k .eq. 1) then
!     print *, "k = 1, rlev = ", rlev
!     print *, "STEVE: STOPPING ON PURPOSE in common_obs_mom6.f90..."
