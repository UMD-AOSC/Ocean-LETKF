PROGRAM obsop_sst_acspo
  ...
  if (BINARY_INPUT) then ![processed binary input]
     print *, "obsop_sst_acspo.f90:: reading binary file=",trim(obsinfile)
     CALL get_nobs(trim(obsinfile),8,nobs,errIfNoObs=.false.)

     if (nobs<=0) then
        print*, "obsop_sst_acspo.f90: nobs=",nobs,"<=0. Exit now..."
        STOP (0)
     endif
     ...
     print *, "obsop_sst_acspo.f90:: reading nc file=",trim(obsinfile)
     ...
     if (nobs<0) then
        print*, "obsop_sst_acspo.f90: nobs<=0. Exit now..."
        STOP (0)
     endif
     ...
     print *, "obsop_sst_acspo.f90:: starting nobs = ", nobs
     do i=1,nobs
