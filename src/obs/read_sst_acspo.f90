MODULE read_sst_acspo
  ...
  PUBLIC :: sst_acspo_data
  PUBLIC :: read_acspo_nc
  ...
  END TYPE sst_acspo_data

CONTAINS

SUBROUTINE read_acspo_nc(obsinfile, min_quality_level, obs_data, nobs, validId, Syyyymmddhh, delta_seconds)
  ...
  TYPE(sst_acspo_data),ALLOCATABLE,INTENT(INOUT) :: obs_data(:)
  INTEGER,     INTENT(OUT) :: nobs
  CHARACTER(*), INTENT(IN) :: validId ! validation id from global attr e.g."AVHRRF_MA-STAR-L2P-v2.8". default is "NONE" for no check, otherwise check if file id equals validId. Throw errors if unmatch
...
!-------------------------------------------------------------------------------
  ...
  CALL nc_rdatt(fid, "","id", acspoId)
  WRITE(6,*) "[msg] read_acspo_nc::id from file  =", trim(acspoId)
  WRITE(6,*) "[msg] read_acspo_nc::id from user  =", trim(validId)
  if (trim(validId) .ne. "NONE") then
     if (trim(validId) .ne. trim(acspoId)) then
        WRITE(6,*) "[err] validId and file Id does not match."
        STOP (25)
     endif
  endif
  ...
  WRITE(6,*) "[msg] read_acspo_nc::ni, nj =", ni, nj
  ...
  WRITE(6,*) "[msg] read_acspo_nc::alon2d: min, max=", &
             minval(alon2d),maxval(alon2d)
  CALL nc_rdvar2d(fid,"lat",alat2d)
  WRITE(6,*) "[msg] read_acspo_nc::alat2d: min, max=", &
...
!-------------------------------------------------------------------------------
  ...
  WRITE(6,*) "[msg] read_acspo_nc::sst: min, max=", &
  ...
  WRITE(6,*) "[msg] read_acspo_nc::sst: Converting from K to degC"
  WRITE(6,*) "[msg] read_acspo_nc::sst: min, max=", &
             minval(sea_surface_temperature), maxval(sea_surface_temperature), &
             minval(sea_surface_temperature, mask=valid),&
             maxval(sea_surface_temperature, mask=valid)
...
! SST error
  ...
  WRITE(6,*) "[msg] read_acspo_nc::stde: min, max=", &
...
!-------------------------------------------------------------------------------
  ...
  WRITE(6,*) "[msg] read_acspo_nc::time=",time(1)
  sst_time = time(1)
  ...
  WRITE(6,*) "[msg] read_acspo_nc::obstime=", obsdate(1:3),obsdate(5:7)
  call W3FS21(sdate(1:5), sdate_in_min_since19780101)
  sdate_in_seconds_since19780101 = 60 * sdate_in_min_since19780101
  WRITE(6,*) "[msg] read_acspo_nc::obstime_since_19780101=", sdate_in_seconds_since19780101
...
! sst_dtime
  ...
  WRITE(6,*) "[msg] read_acspo_nc::sst_dtime: min, max=", &
  ...
  WRITE(6,*) "[msg] read_acspo_nc::sst_dtime_since19780101: min, max=", &
  ...
  if (PRESENT(Syyyymmddhh) .and. PRESENT(delta_seconds)) then
     ...
     WRITE(6,*) "[msg] read_acspo_nc::Syyyymmddhh=", Syyyymmddhh, &
                "qcdate(:)=",qcdate(:)
     call W3FS21(qcdate,qcdate_in_min_since19780101)
     qcdate_in_seconds_since19780101 = 60 * qcdate_in_min_since19780101
     WRITE(6,*) "[msg] read_acspo_nc::qcdate_in_seconds_since19780101, delta_seconds=", &
...
!-------------------------------------------------------------------------------
  ...
  if (dodebug) then
     ...
     do i = -1, 5
        ...
        n = sum(int(i1buf2d,4))
        WRITE(6,*) "[msg] read_acspo_nc::quality_level, # of obs, %=", i, n, n*100.0/(ni*nj)
     end do
  end if
  where (quality_level == FillValue)
      valid = .false.
  end where
  !WRITE(6,*) "[msg] read_acspo_nc::quality_level: min, max=", &
...
! determine num of valid obs
  WRITE(6,*) "[msg] read_acspo_nc::use obs with min_quality_level=", min_quality_level
  nobs = 0
  do j = 1, nj; do i = 1, ni
  ...
  WRITE(6,*) "[msg] read_acspo_nc::nobs_retained, %=", nobs, nobs*100.0/(ni*nj)
...
! fill into data struct
  ...
  if (n/=nobs) then
     WRITE(6,*) "[err] read_acspo_nc::n/=nobs: n, nobs=", n, nobs
     STOP (26)
  end if

END SUBROUTINE read_acspo_nc

SUBROUTINE inspect_obs_data(obs_data)
  IMPLICIT NONE
  TYPE(sst_acspo_data),INTENT(IN) :: obs_data(:)

  WRITE(6,*) "[msg] read_acspo_nc::info"
