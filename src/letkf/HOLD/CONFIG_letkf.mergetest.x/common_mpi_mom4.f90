...
SUBROUTINE read_ens_mpi(file,member,v3d,v2d)
  ...
  DO l=1,ll
    ...
    IF(im <= member) THEN
      ...
      WRITE(6,'(A,I3.3,2A)') 'In common_mpi_oceanmodel.f90::read_ens_mpi, MYRANK ',myrank,' is reading a file ',filename
...
!     if (.false.) CALL write_grd4('test.'//filename,v3dg,v2dg) 
    ...
    if (dodebug) WRITE(6,*) "In common_mpi_oceanmodel.f90::read_ens_mpi, calling scatter_grd_mpi_alltoall..."
...
! INCLUDE 'netcdf.inc' !STEVE: for NaN correction (OCEAN)
  ...
  DO l=1,ll
    ...
    IF(im <= member) THEN
      ...
      !STEVE: debug
!     WRITE(6,*) "common_mpi_oceanmodel.f90::write_ens_mpi:: MAXVAL(ABS(v3dg(:,:,:,iv3d_t))) = ", MAXVAL(ABS(v3dg(:,:,:,iv3d_t)))
!     WRITE(6,*) "common_mpi_oceanmodel.f90::write_ens_mpi:: MAXVAL(ABS(v3dg(:,:,:,iv3d_s))) = ", MAXVAL(ABS(v3dg(:,:,:,iv3d_s)))
...
SUBROUTINE write_ens_mpi_grd(file,member,v3d,v2d)
  ...
  DO l=1,ll
    ...
    IF(im <= member) THEN
      ...
      !STEVE: debug
!     print *, "common_mpi_oceanmodel.f90::write_ens_mpi_grd:: MAXVAL(ABS(v3dg(:,:,:,iv3d_t))) = ", MAXVAL(ABS(v3dg(:,:,:,iv3d_t)))
