...
!   ai = (rrlon - lon(nlon)) / (lon(1) - lon(nlon)) !STEVE: deal with wrap - fix this later
  ...
  if (dodebug .and. (ri > nlon .or. ri < 1)) then
    WRITE(6,*) "In common_obs_oceanmodel.f90::phys2ijk," 
    WRITE(6,*) "rlon = ", rlon
    WRITE(6,*) "rrlon = ", rrlon
    WRITE(6,*) "ai = ", ai
    WRITE(6,*) "ri = ", ri
...
!
  ...
  elseif (NINT(elem) == id_sss_obs) then ! surface observation !(OCEAN)
    ...
    if (k .eq. 1) then
!     print *, "k = 1, rlev = ", rlev
!     print *, "STEVE: STOPPING ON PURPOSE in common_obs_oceanmodel.f90..."
...
!   if (.true.) then
!     print *, "common_obs_oceanmodel.f90::itpl_2d::"
...
!===============================================================================
  ...
  do n=1,nn
    ...
    !STEVE: error check
    if (oerr(n) .le. 0) then
      print *, "common_obs_oceanmodel.f90::read_obs:: WARNING!"
