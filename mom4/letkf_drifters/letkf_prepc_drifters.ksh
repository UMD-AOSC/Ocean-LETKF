...
#LUYU: add drifters folder
workdir_fcst_drifters=${EXP_DATA}/${YYYYMMDDHH}/model/${MEMBERID}/DRIFTERS
...
IY=${YYYYMMDDHH:0:4} #LUYU: IY represents the initial year
IM=${YYYYMMDDHH:4:2} #LUYU: IM represents the initial month
ID=${YYYYMMDDHH:6:2} #LUYU: ID represents the initial day
IH=${YYYYMMDDHH:8:2} #LUYU: IH represents the initial hour
IN=00 #??
IS=00 #??

FCST=0
while test $FCST -lt $NSLOTS	#LUYU: ?? What is $NSLOTS 
do
  FCST=`expr $FCST + 1`	
  ...
  # LUYU: link the background drifters model data
  ln -f $workdir_fcst/$IY$IM$ID.$IH$IN$IS.drifters_inp.nc            gs${ISLOT2}$MEM3.drifters_inp.nc
...
# done
  ...
  if [ "${ISLOT2}" -eq "${ATIME}" ]; then #STEVE: trying to only do it once per member...
    ...
    #LUYU: add drifters
    cp ${workdir}/gs${ISLOT2}$MEM3.drifters_inp.nc        ${workdir2}/anal$MEM3.drifters_inp.nc
  fi
  ...
  #LUYU: add drifters
  ln -f ${workdir}/gs${ISLOT2}$MEM3.drifters_inp.nc gues.drifters_inp.nc
...
# $OBSOPexe -obsin $OBSDIR/$IY$IM$ID.dat -gues gs${ISLOT2}$MEM3 -obsout ${workdir2}/obs${ISLOT2}${MEM3}.dat > obsope.log
  ...
  ln -f ${workdir}/gs${ISLOT2}$MEM3.drifters_inp.nc        ${workdir2}/gs${ISLOT2}$MEM3.drifters_inp.nc
# ls ${workdir2}
