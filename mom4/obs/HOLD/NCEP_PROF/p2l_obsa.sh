...
INDIR1=~/lf1u/OBS/historical/TMP_profs_tmpa
INDIR2=~/lf1u/OBS/historical/SAL_profs_O_sala
OUTDIR=~/lf1u/OBS/historical/letkf_fmt/PROFS_gerr_TS
mkdir -p $INDIR1
mkdir -p $INDIR2
mkdir -p $OUTDIR
#ZIPDIR=$CWD/ARCS
...
do
  ...
  if [ 1 -eq 1 ]; then
    ...
    rm -f tmpa.mom
    $scriptDir/$oaTexe -f tmpFiles salFiles -o tmpa.mom -g grid_spec.nc -k 30
    ...
    if [ -f $arcdir_so/${IY}${IM}${ID}sal.nc ]; then
      ...
      rm -f sala.mom
      $scriptDir/$oaSexe -f salFiles -o sala.mom -g grid_spec.nc -k 30
