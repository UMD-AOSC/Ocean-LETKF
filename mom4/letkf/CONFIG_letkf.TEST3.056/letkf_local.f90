...
!===============================================================================
  ...
  USE params_letkf, ONLY: nbv, DO_NO_VERT_LOC, localization_method, DO_IRREG_GRID
...
! LOGICAL :: splitarc = .true.  ! initialize for splitting the arctic with (TRIPOLAR) grid
  !STEVE:
...
! REAL(r_size) :: minlon_old,maxlon_old,minlat_old,maxlat_old
  ...
  if( nobs > 0 ) then
    ...
    WRITE(6,*) "letkf_loca.f90::obs_local NO OBSERVATIONS!"
    STOP(695)
  endif
  wbdy=lon0
  ebdy=lonf
...
! endif
  ...
  ! this isn't really local anymore, but still, we'll cover it
...
!SUBROUTINE wrap_orig
!
!  !-----------------------------------------------------------------------------
!  ! Handle the case when the bounding box is split on the edge of the grid.
!  !-----------------------------------------------------------------------------
!  !STEVE: ISSUE
...
!  splitlat = .false.
!
!  if (maxlon - minlon > lonf - lon0) then  ! The 'local' region overlaps the whole globe
!    fulllon = .true.
!    if (dodebug) then 
!      WRITE(6,*) "FullLon."
!      WRITE(6,*) "xlon = ", xlon
...
!    maxlon = lonf
!
!  else ! The local region either partially overlaps global boundaries, or is entirely contained
!
...
!      WRITE(6,*) "letkf_local.f90::obs_local: Grid point is close to pole (cutting localization region at northern boundary)."
!    endif
!  endif
!
!
