...
! 
!-------------------------------------------------------------------------------
! $Authors: Steve Penny, Travis Sluka (kd-tree implementation and strong coupling) $
!===============================================================================
...
SUBROUTINE obs_local(ij,ilev,mlev,var_local,hdxf,rdiag,rloc,dep,nobsl,nobstotal)
  ...
  ! Variables used internally for hz localization
  ...
  if (initialized == 0) then
    ...
    initialized = 1
    call kd_init(kdtree_root, obslon(1:nobs), obslat(1:nobs))
