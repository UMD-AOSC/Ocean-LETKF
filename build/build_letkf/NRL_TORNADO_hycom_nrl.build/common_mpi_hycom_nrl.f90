...
!   12/15/2017 Steve Penny updated for use with NCODA
!
!=======================================================================
  USE common
  USE common_mpi
...
!===============================================================================
  ...
  v2dg=0.0
  do j=1,nlat
    ! 2D and 1D Data stored layers:
    v2dg(:,j,1) = SNGL(dx(:,j))
    v2dg(:,j,2) = SNGL(dy(:,j))
    v2dg(:,j,3) = SNGL(lon2d(:,j))
    v2dg(:,j,4) = SNGL(lat2d(:,j))
    v2dg(:,j,5) = SNGL(kmt0(:,j))
    v2dg(:,j,6) = SNGL(phi0(:,j))  !STEVE: Placeholder for additional 2d variable
...
!===============================================================================
  ...
  ALLOCATE(tmp(nij1max,nprocs))
  ALLOCATE(bufs(mpibufsize))
  ALLOCATE(bufr(mpibufsize,nprocs))
...
SUBROUTINE read_ens_mpi(infile,nens,v3d,v2d)
!===============================================================================
