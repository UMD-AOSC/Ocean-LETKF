...
!
! DESCRIPTION: 
!   This program acts as the observation operator for all obs data from NCODA-prep
...
!
! USAGE:
!   A separate instance is run independently for each member
!   All observations are read in and converted to the letkf obs2 format w/ H(x) data 
!   for each member added in a new column.
! 
! !REVISION HISTORY:
!   12/22/2017 Steve Penny set up for reading NCODA prep '_prp' and '_ENS_obs' files
...
!===============================================================================
  ...
  USE vars_obs
  USE common_obs_oceanmodel
  USE read_ncoda_prep,           ONLY: ncoda_prep_data, read_ncoda_prp
  ...
  INTEGER :: obid = id_t_obs    ! Id number for observation type (defined in params_obs.f90)
                                ! Default: id_t_obs: reads both temperature and
                                ! salinity profiles (id_t_obs and id_s_obs)
  ...
  !-----------------------------------------------------------------------------
  print *, "obsop_ncoda_3d.f90:: calling read_ncoda_prp observations..."
  CALL read_ncoda_prp(obsinfile,ensinfile,obs_data,nobs,mem,obid)
  print *, "obsop_ncoda_3d.f90:: finished read_ncoda_prp."
  ...
  !-----------------------------------------------------------------------------
  print *, "obsop_ncoda_3d.f90:: starting nobs = ", nobs
  do i=1,nobs
  ...
  !-----------------------------------------------------------------------------
  WRITE(6,*) "In obsop_ncoda_3d.f90:: observations removed for:"
